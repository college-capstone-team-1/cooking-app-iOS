...
class FavoriteViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        dest.recipeTuple = (seq:recipeData[row]?.RCP_SEQ,
                            ...
                            //만드는법 배열
                            manual:[recipeData[row]?.MANUAL01, recipeData[row]?.MANUAL02, recipeData[row]?.MANUAL03, recipeData[row]?.MANUAL04, recipeData[row]?.MANUAL05, recipeData[row]?.MANUAL06, recipeData[row]?.MANUAL07, recipeData[row]?.MANUAL08, recipeData[row]?.MANUAL09, recipeData[row]?.MANUAL10].filter{ $0 != nil }.map{ Optional($0!.components(separatedBy: "\n").joined())},
                            //만드는법 사진링크 배열
                            manualImg:[recipeData[row]?.MANUAL_IMG01, recipeData[row]?.MANUAL_IMG02, recipeData[row]?.MANUAL_IMG03, recipeData[row]?.MANUAL_IMG04, recipeData[row]?.MANUAL_IMG05, recipeData[row]?.MANUAL_IMG06, recipeData[row]?.MANUAL_IMG07, recipeData[row]?.MANUAL_IMG08, recipeData[row]?.MANUAL_IMG09, recipeData[row]?.MANUAL_IMG10].filter{ $0 != nil }
                            )
    }
